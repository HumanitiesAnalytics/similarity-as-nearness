...
1. Try Rendering the Quarto document to HTML to make sure sure everything works. 
2. Run each code chunk individually and look at the results. 
3. Use various tools (`?` in RStudio generative AI, online resources, etc.) to investigate the questions. below. Take written or typed notes as you go.
4. After a few minutes, the professor will signal you to discuss all items with a classmate.
5. Lastly, we will discuss everything as a class.
...
labels <- c("Root Excerpt", "Atlantic review of \n\'Breakfast of Champions\'\n(Candidate 1)\n", "Nora Sayre\'s review of \n\'The Queen Was in the Garbage\'\n(Candidate 2)\n", "Computer & Gaming World review of \n\'Sam & Max Hit the Road\'\n(Candidate 3)\n")

reading_level_plot <- reading_level %>% 
  mutate(text = labels) %>% 
  mutate(text = factor(text, levels = text, ordered = TRUE))
...
ggplot(reading_level_plot_candidates, aes(x=syllables, y=sentence_len, color=text, shape=text)) + 
  geom_point(size = 3.5,) + 
  ...
  geom_point(aes(x=root$syllables, y=root$sentence_len), color='black', shape='*', size=14) + 
  annotate("text", 
           x=root$syllables, 
  ...
  annotate("text", 
           ...
           fontface = "bold") +
  annotate("text", 
           x = root$syllables - 0.023, 
           y = c_1$sentence_len + 9, 
           label = "Euclidean Distance", 
           color = "orange", 
           size = 5.5, 
           fontface = "bold") +
  theme(plot.title = element_text(size = 18), plot.caption = element_text(hjust = 0), legend.text = element_text(size = 14),  legend.title = element_text(size = 18))

```
```{r}

# visualize 2D (cosine vectors)

ggplot(reading_level_plot_candidates, aes(x=syllables, y=sentence_len, color=text, shape=text)) + 
  geom_point(size=3.5) + 
  labs(title = "Book Review Cosine Similarity (Excerpts)\n", 
       x = "Average syllables per word", 
       y = "Average sentence length", 
       color = "Excerpt\n",
       shape = "Excerpt\n",
       caption = "Figure 2: Similarity comparison of Nora Sayre's review of \'Breakfast of Champions\' (Root Excerpt) \nto three other book review excerpts (Candidates 1-3), this time visualizing similarity as directional vectors") +
  geom_point(aes(x=root$syllables, y=root$sentence_len), color='black', shape='*', size=12) + 
  annotate("text", 
           x=root$syllables, 
           y=root$sentence_len + 2, 
           label = "Root Excerpt", 
           color = "black", 
           size = 5.5, 
           fontface = "bold") +
...
## Questions for Discussion (Take Some Notes)

1. How is `philentropy::distance` being used on lines 60 and 79? What is the output when this function is run?  
2. How are results being visualized using the `ggplot()` function? Why do the results look so different when you render to HTML? 
3. Take a look at the variables `reviews_full` and `excerpts_full`. How well do the excerpts seem to represent the reviews they are taken from? 
4. Take a look at the variables `reviews_no_hapax` and `excerpts_no_hapax`. What do these variables represent? 
